...
def _assign_crowding_dists_in_front(pareto_front, env, complexity_bounds):
    ...
    for objective_func in (_perf_objective, _complexity_objective):
        ...
        for idx in range(1, (num_items - 1)):
            ...
            fmax = _get_fmax(objective_func, env, complexity_bounds)
            fmin = _get_fmin(objective_func, env, complexity_bounds)
...
def _get_fmax(objective_func, env, complexity_bounds):
    if objective_func == _perf_objective:
        return env.max_perf
    elif objective_func == _complexity_objective:
        return complexity_bounds.max
    else:
        assert False


def _get_fmin(objective_func, env, complexity_bounds):
    if objective_func == _perf_objective:
        return env.min_perf
    elif objective_func == _complexity_objective:
        return complexity_bounds.min
    else:
        assert False
...
def crowded_comparison_sort(container):
    crowding_dist_desc = sorted(container,
                                ...
                                reverse=True)
    return sorted(crowding_dist_desc, key=lambda item: item.pareto_front_rank)
